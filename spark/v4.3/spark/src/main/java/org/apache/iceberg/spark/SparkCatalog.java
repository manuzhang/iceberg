...
public class SparkCatalog extends BaseCatalog {
  ...
  private Set<String> catalogManagedViewProperties = ImmutableSet.of();
  ...
  private View commitView(Identifier ident, View view, ViewCommit viewCommit)
    ...
    try {
      ...
      Set<String> replacedProperties = null;
      switch (viewCommit) {
        case CREATE:
          icebergView = builder.create();
          break;
        case REPLACE:
          replacedProperties = propertiesReplacedBySpark(viewIdentifier);
          icebergView = builder.replace();
          break;
        case CREATE_OR_REPLACE:
          Pair<Set<String>, org.apache.iceberg.view.View> replacement =
              createOrReplaceView(viewIdentifier, builder);
          replacedProperties = replacement.first();
          icebergView = replacement.second();
          break;
        default:
      ...
      if (replacedProperties != null) {
        removeDroppedViewProperties(icebergView, replacedProperties, props.keySet());
      }
  ...
  private Pair<Set<String>, org.apache.iceberg.view.View> createOrReplaceView(
      TableIdentifier viewIdentifier, ViewBuilder builder) {
    try {
      Set<String> replacedProperties = propertiesReplacedBySpark(viewIdentifier);
      return Pair.of(replacedProperties, builder.replace());
    } catch (org.apache.iceberg.exceptions.NoSuchViewException e) {
      return createViewAfterMissing(viewIdentifier, builder);
    }
  }

  private Pair<Set<String>, org.apache.iceberg.view.View> createViewAfterMissing(
      TableIdentifier viewIdentifier, ViewBuilder builder) {
    try {
      return Pair.of(null, builder.create());
    } catch (AlreadyExistsException concurrentCreate) {
      return replaceConcurrentlyCreatedView(viewIdentifier, builder, concurrentCreate);
    }
  }

  private Pair<Set<String>, org.apache.iceberg.view.View> replaceConcurrentlyCreatedView(
      TableIdentifier viewIdentifier,
      ViewBuilder builder,
      AlreadyExistsException concurrentCreate) {
    Set<String> replacedProperties;
    try {
      replacedProperties = propertiesReplacedBySpark(viewIdentifier);
    } catch (org.apache.iceberg.exceptions.NoSuchViewException missingView) {
      concurrentCreate.addSuppressed(missingView);
  ...
  private Set<String> propertiesReplacedBySpark(TableIdentifier identifier) {
    org.apache.iceberg.view.View icebergView = asViewCatalog.loadView(identifier);
    View sparkView = SparkView.toView(catalogName, icebergView);
    Set<String> properties = Sets.newHashSet(sparkView.properties().keySet());
    properties.removeIf(SparkView::isReservedProperty);

    // Avoid treating defaults synthesized by SparkView as previously persisted metadata.
    SparkView.internalProperties(sparkView).keySet().stream()
        .filter(icebergView.properties()::containsKey)
        .forEach(properties::add);
    return properties;
  }

  private void removeDroppedViewProperties(
      org.apache.iceberg.view.View view,
      Set<String> previousProperties,
      Set<String> replacementProperties) {
    // Iceberg replacements merge properties, while Spark replacement views carry the complete
    // property set. ViewBuilder does not support property removals, so this is a separate retrying
    // metadata commit and can fail after the view version has already been replaced.
    Set<String> removals = Sets.newHashSet(previousProperties);
    removals.removeAll(replacementProperties);
    removals.removeAll(ICEBERG_MANAGED_VIEW_PROPERTIES);
    removals.removeAll(catalogManagedViewProperties);
    if (!removals.isEmpty()) {
  ...
  public final void initialize(String name, CaseInsensitiveStringMap options) {
    ...
    Set<String> configuredViewProperties = Sets.newHashSet();
    configuredViewProperties.addAll(
        PropertyUtil.propertiesWithPrefix(
                options.asCaseSensitiveMap(), CatalogProperties.VIEW_DEFAULT_PREFIX)
            .keySet());
    configuredViewProperties.addAll(
        PropertyUtil.propertiesWithPrefix(
                options.asCaseSensitiveMap(), CatalogProperties.VIEW_OVERRIDE_PREFIX)
            .keySet());
    this.catalogManagedViewProperties = ImmutableSet.copyOf(configuredViewProperties);
